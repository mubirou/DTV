...
1. ノードを選択→右クリック→[ノードを追加]→[シリアルノードを追加]  
    （設定のレイヤーのようなもの）
1. ノード01を選択し、次の設定で明るさを調整（「プライマリーホイール」→「Log」に変更／スコープを見ながら調整）  
    * シャドウ（初期値0.00）：暗部の明るさ
...
[再生]-[レンダーキャッシュを削除]-[すべて]
* キャッシュファイルのコーデックの選択  
[ファイル]-[プロジェクト設定]-[マスター設定]-[最適化メディア＆レンダーキャッシュ]-[レンダーキャッシュのフォーマット]: DNxHD HQX（12bit 4:2:2）
* 自動レンダリングの開始時間の設定  
[ファイル]-[プロジェクト設定]-[マスター設定]-[最適化メディア＆レンダーキャッシュ]-[次の数秒後にバックグラウンドキャッシュを開始]: 3秒（初期値5秒）

#### オーディオクリップの速度の変更
1. 速度を変更したいクリップを選択
1. 右クリック→[クリップの速度を変更]
1. [速度]を変更（初期値100.00%）  
1. 音程は変更したくない場合は[ピッチ補正]を✔（初期値）
